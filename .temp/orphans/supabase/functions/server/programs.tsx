...
// Get public programs
app.get('/make-server-f0b2daa4/programs/public', async (c) => {
  try {
...
app.get('/make-server-f0b2daa4/programs/public/:slug', async (c) => {
  try {
...
app.post('/make-server-f0b2daa4/programs/:programId/check-availability', async (c) => {
  try {
...
app.post('/make-server-f0b2daa4/programs/:programId/book', async (c) => {
  try {
    const { user, error: authError } = await verifyAuth(c.req.raw);
    if (authError || !user) {
      return c.json({ error: authError || 'Authentication required' }, 401);
...
app.get('/make-server-f0b2daa4/programs/my-bookings', async (c) => {
  try {
    const { user, error: authError } = await verifyAuth(c.req.raw);
    if (authError || !user) {
      return c.json({ error: authError || 'Authentication required' }, 401);
...
app.get('/make-server-f0b2daa4/admin/programs', async (c) => {
  try {
...
app.post('/make-server-f0b2daa4/admin/programs', async (c) => {
  try {
    const { user, orgUser, error: authError } = await verifyAuth(c.req.raw);
    if (authError || !orgUser) {
      return c.json({ error: authError || 'Admin access required' }, 401);
...
app.put('/make-server-f0b2daa4/admin/programs/:programId', async (c) => {
  try {
    const { user, orgUser, error: authError } = await verifyAuth(c.req.raw);
    if (authError || !orgUser) {
      return c.json({ error: authError || 'Admin access required' }, 401);
...
app.get('/make-server-f0b2daa4/admin/programs/:programId/bookings', async (c) => {
  try {
    const { user, orgUser, error: authError } = await verifyAuth(c.req.raw);
    if (authError || !orgUser) {
      return c.json({ error: authError || 'Admin access required' }, 401);
...
app.put('/make-server-f0b2daa4/admin/bookings/:bookingId/status', async (c) => {
  try {
    const { user, orgUser, error: authError } = await verifyAuth(c.req.raw);
    if (authError || !orgUser) {
      return c.json({ error: authError || 'Admin access required' }, 401);
