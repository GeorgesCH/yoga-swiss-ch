...
app.get('/make-server-f0b2daa4/retreats/public', async (c) => {
  try {
...
app.get('/make-server-f0b2daa4/retreats/public/:slug', async (c) => {
  try {
...
app.post('/make-server-f0b2daa4/retreats/:retreatId/lead-capture', async (c) => {
  try {
...
app.post('/make-server-f0b2daa4/retreats/:retreatId/apply', async (c) => {
  try {
    const retreatId = c.req.param('retreatId');
...
app.get('/make-server-f0b2daa4/retreats/my-applications', async (c) => {
  try {
...
app.get('/make-server-f0b2daa4/admin/retreats/:retreatId/applications', async (c) => {
  try {
...
app.put('/make-server-f0b2daa4/admin/applications/:applicationId/review', async (c) => {
  try {
    const { user, orgUser, error: authError } = await verifyAuth(c.req.raw);
    if (authError || !orgUser) {
      return c.json({ error: authError || 'Admin access required' }, 401);
...
app.post('/make-server-f0b2daa4/admin/retreats', async (c) => {
  try {
    const { user, orgUser, error: authError } = await verifyAuth(c.req.raw);
    if (authError || !orgUser) {
      return c.json({ error: authError || 'Admin access required' }, 401);
...
// Simple health check
app.get('/make-server-f0b2daa4/retreats/health', (c) => {
  return c.json({ 
