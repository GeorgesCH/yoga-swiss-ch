...
// GET / - Get user's organizations with hybrid support
app.get("/", async (c) => {
  try {
    ...
    try {
      ...
      if (!supabaseError && supabaseOrgs && Array.isArray(supabaseOrgs) && supabaseOrgs.length > 0) {
        userOrgs = supabaseOrgs.map(org => ({
          ...
          name: org.name,
          slug: org.slug,
          type: org.type || 'studio', // Default to studio if type is missing
...
// POST / - Create new organization with dual storage
app.post("/", async (c) => {
  let orgId = null;
  let slug = null;
  ...
  try {
    ...
    const { name, type, settings = {}, parent_org_id } = body;
    slug = body.slug;
    ...
    if (parent_org_id) {
      // Check if parent organization exists and is a brand
      ...
      if (parentError || !parentOrg) {
        return c.json({ error: 'Parent organization not found' }, 400);
      }

      // Enforce constraint: studios can only be children of brands
      if (type === 'studio' && parentOrg.type !== 'brand') {
        return c.json({ 
          error: 'Studios can only be children of brands, not other studios' 
        }, 400);
      }
    }
    ...
    try {
      console.log(`[Orgs] Calling create_organization_owner RPC with: name=${name}, slug=${slug}, type=${type}, parent_org_id=${parent_org_id}`);
      const { data: createdOrgId, error: supabaseError } = await supabase
        .rpc('create_organization_owner', {
          p_name: name,
          p_slug: slug,
          p_type: type,
          p_parent_org_id: parent_org_id || null,
          p_locale: 'de-CH',
